...
public class MainActivity extends FragmentActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (appStartedFromShortcut())
			browseHandler.openShortcut(new File(getIntent().getStringExtra(
					"shortcut_path")));
		else
	...
	public void onBackPressed() {
		...
		// 1 can be either at the home/root folder, or after a dialog item click
		// (History/Favorites)
	...
	private void handleIfSdCardStateChanged() {
		// if inside sd card & sd card isnt mounted - show "sd card unmounted"
		// view
		if (BrowseHandler.isSdCardMounted() == false
			...
			if (browseHandler.isSdCardUnmountedViewShown())
				browseHandler.undisplaySdCardUnmountedView(new File(
						BrowseHandler.current_path));
		}
	}
	...
	public void pasteButton(View vw) {
		...
		if (success)
			...
			Toast.makeText(this, "Paste didn't finish successfully.",
	...
	public void showAdvancedSearchButton(View vw) {
		...
		LinearLayout advanced_search = (LinearLayout) ((View) vw.getParent())
				.findViewById(R.id.advanced_search);
